...
with row2_1:
    ...
    fig = px.scatter(tum, x = [sicaklik_x if eksen_x == 'Sıcaklık' else yagis_x if eksen_x == 'Yağış' else diger_x][0]+str(ay_x), y = [sicaklik_y if eksen_y == 'Sıcaklık' else yagis_y if eksen_y == 'Yağış' else diger_y][0]+str(ay_y), animation_frame='yil', size = arpa_bugday, color = 'Bölge',
          ...
          template = 'none',
          size_max=60,
          labels = {'Ort. Günlük Sıcaklık Farkı' + str(ay_x): 'Ortalama günlük sıcaklık farkı ' + str(ay_x), 'GOrtSıcOrt' + str(ay_x): 'Günlük ortalama sıcaklık ortalaması ' + str(ay_x), 'GMinSıcOrt' + str(ay_x): 'Günlük minimum sıcaklık ortalaması ' + str(ay_x), 'GMinSıcMin' + str(ay_x): 'Günlük minimum sıcaklığın minimumu ' + str(ay_x), 'GMinSıcMax' + str(ay_x): 'Günlük minimum sıcaklığın maxsimumu ' + str(ay_x), 'GMaxSıcOrt' + str(ay_x): 'Günlük maksimum sıcaklık ortalaması ' + str(ay_x), 'GMaxSıcMin' + str(ay_x): 'Günlük maksimum sıcaklığın minimumu ' + str(ay_x), 'MaxGMaxSıc' + str(ay_x): 'Maksimum günlük sıcaklığın maksimumu ' + str(ay_x),
                   'Ort. Günlük Sıcaklık Farkı' + str(ay_y): 'Ortalama günlük sıcaklık farkı ' + str(ay_y), 'GOrtSıcOrt' + str(ay_y): 'Günlük ortalama sıcaklık ortalaması ' + str(ay_y), 'GMinSıcOrt' + str(ay_y): 'Günlük minimum sıcaklık ortalaması ' + str(ay_y), 'GMinSıcMin' + str(ay_y): 'Günlük minimum sıcaklığın minimumu ' + str(ay_y), 'GMinSıcMax' + str(ay_y): 'Günlük minimum sıcaklığın maxsimumu ' + str(ay_y), 'GMaxSıcOrt' + str(ay_y): 'Günlük maksimum sıcaklık ortalaması ' + str(ay_y), 'GMaxSıcMin' + str(ay_y): 'Günlük maksimum sıcaklığın minimumu ' + str(ay_y), 'MaxGMaxSıc' + str(ay_y): 'Maksimum günlük sıcaklığın maksimumu ' + str(ay_y)}
          )
